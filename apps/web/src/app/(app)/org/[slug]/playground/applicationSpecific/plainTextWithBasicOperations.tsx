...
function textOperationToAggregatedBasicTextOperation(
  ...
  let pos = 0
  for (const op of textOperation.ops) {
    if (typeof op === 'string') {
...
function aggregatedBasicTextOperationToTextOperation(
  ...
  for (const basicTextOperation of aggregatedBasicTextOperation) {
      ...
      case BasicTextOperationType.Delete:
        textOperation = textOperation.compose(
          new TextOperation()
            ...
            .retain(
              textLength -
                basicTextOperation.position -
                basicTextOperation.length,
            ),
        )
...
const CodeMirrorComponent =
  makeCodeMirrorComponent<AggregatedBasicTextOperation>(
    ...
    (changes, editor) => {
      const [operation] = CodeMirrorAdapter.operationFromCodeMirrorChanges(
        changes,
        editor,
      )
  ...
  if (a.type === BasicTextOperationType.Insert) {
    if (b.type === BasicTextOperationType.Insert) {
      ...
      if (a.position <= b.position) {
        return [
          [a],
          [makeInsertOperation(b.position + a.content.length, b.content)],
        ]
      } else {
        return [
          [makeInsertOperation(a.position + b.content.length, a.content)],
          [b],
        ]
      }
    } else {
...
> = {
  ...
  compose(
    first: AggregatedBasicTextOperation,
    second: AggregatedBasicTextOperation,
  ) {
    return [...first, ...second]
  },
  apply(operation: AggregatedBasicTextOperation, snapshot: string) {
    return aggregatedBasicTextOperationToTextOperation(
      operation,
      snapshot.length,
    ).apply(snapshot)
  },
