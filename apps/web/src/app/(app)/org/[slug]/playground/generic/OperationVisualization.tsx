...
const useOperationStyles = () => {
  return {
    operation: 'inline-block w-5 h-5 rounded-full', // width/height 20px = 5 * 4px, borderRadius 10px = rounded-full
    tooltip: 'text-[13px]', // font-size 13px
  }
}
...
const isRelated = (
  a: OperationWithoutPayload,
  b: OperationWithoutPayload,
): boolean => a.meta.id === b.meta.id
...
const renderRelatedOperation = <OpT extends unknown>(
  ...
  if (
    isOneArrayPrefixOfTheOther(
      operation.transformedAgainst,
      relatedOperation.transformedAgainst,
    )
  ) {
    const transformedAgainstDifference =
      operation.transformedAgainst.length -
      relatedOperation.transformedAgainst.length
...
export type OperationVisualizationComp<OpT> = FunctionComponent<
  OperationProps<OpT>
>
...
export const makeOperationVisualization =
  ...
  (props) => {
    ...
    const { className, style, operation, tooltipPlacement, ...otherProps } =
      props

    const [tooltipContent, setTooltipContent] =
      useState<NonNullable<React.ReactNode>>('')
    const [selfOpenStatus, setSelfOpenStatus] = useState<SelfOpenStatus>(
      SelfOpenStatus.Closed,
    )
    ...
    useEffect(() => {
      if (
        ...
        selfOpenStatus === SelfOpenStatus.Closing &&
        hoveredOperation === undefined
      ) {
    ...
    const onOpen = useCallback(() => {
      ...
      setTooltipContent(
        renderOperation(applicationSpecific.renderOperation, operation),
      )
    ...
    const onClose = useCallback(() => {
      setSelfOpenStatus(
        hoveredOperation ? SelfOpenStatus.Closing : SelfOpenStatus.Closed,
      )
    ...
    return (
      <TooltipProvider>
        <Tooltip
          open={
            selfOpenStatus === SelfOpenStatus.Open || isRelatedOperationHovered
          }
          onOpenChange={() => {
            if (
              selfOpenStatus === SelfOpenStatus.Open ||
              isRelatedOperationHovered
            ) {
              onClose()
            } else {
              onOpen()
        ...
        >
          ...
          <TooltipContent className={`data-[side=${tooltipPlacement}]`}>
            {tooltipContent}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
