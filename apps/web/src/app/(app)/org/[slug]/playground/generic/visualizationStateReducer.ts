...
function receiveOperationFromClient<SnapshotT, OpT>(
  functions: Pick<
    ApplicationSpecificFunctions<SnapshotT, OpT>,
    'transform' | 'apply'
  >,
  ...
  const transformedOperation = concurrentOperations.reduce<
    OperationAndRevision<OpT>
  >(
    (op, concurrentOp) =>
      transformOperation(functions.transform, concurrentOp, op)[1],
    operation,
  )
...
export const makeYouLens = <SnapshotT, OpT>(): Lens<
  VisualizationState<SnapshotT, OpT>,
  ClientAndSocketsVisualizationState<SnapshotT, OpT>
> => ({
  get: (globalState) => globalState.You,
  set: (globalState, YouState) => ({ ...globalState, You: YouState }),
})

export const makeJohnLens = <SnapshotT, OpT>(): Lens<
  VisualizationState<SnapshotT, OpT>,
  ClientAndSocketsVisualizationState<SnapshotT, OpT>
> => ({
  get: (globalState) => globalState.john,
  set: (globalState, johnState) => ({ ...globalState, john: johnState }),
})
...
function processClientUserOperation<OpT>(
  ...
  switch (synchronizationState.status) {
    ...
    case SynchronizationStateStatus.AWAITING_OPERATION: {
      ...
      const buffer: OperationAndRevision<OpT> = {
        ...createNewOperation(baseOperation, clientName),
        revision,
      }
      return {
    ...
    case SynchronizationStateStatus.AWAITING_OPERATION_WITH_BUFFER: {
      return {
        newSynchronizationState: {
          status: SynchronizationStateStatus.AWAITING_OPERATION_WITH_BUFFER,
          awaitedOperation: synchronizationState.awaitedOperation,
          buffer: composeOperation(
            compositionFunction,
            synchronizationState.buffer,
            baseOperation,
          ),
        },
        operationsToSendToServer: [],
...
function getLatestSynchronizationState<SnapshotT, OpT>({
  ...
  return clientLog.length > 0
    ? clientLog[0].newState
    : initialSynchronizationState
}
...
function clientUserOperation<SnapshotT, OpT>(
  ...
  return {
    clientLog: [
      { entry: newClientLogEntry, newState: newSynchronizationState },
      ...clientLog,
    ],
...
export function onServerReceive<SnapshotT, OpT>(
  ...
  const nextVisualizationState = clientLens.set(
    visualizationState,
    newClientState,
  )
  ...
  return {
    ...
    You: sendOperationToClient(
      nextVisualizationState.You,
      operationToBroadcast,
    ),
    john: sendOperationToClient(
      nextVisualizationState.john,
      operationToBroadcast,
    ),
  }
}
  ...
  switch (synchronizationState.status) {
    ...
    case SynchronizationStateStatus.AWAITING_OPERATION: {
      ...
      if (receivedOperation.meta.id === awaitedOperation.meta.id) {
        ...
        const [transformedReceivedOperation, transformedAwaitedOperation] =
          transformOperation(
            transformationFunction,
            receivedOperation,
            awaitedOperation,
    ...
    case SynchronizationStateStatus.AWAITING_OPERATION_WITH_BUFFER: {
      ...
      if (receivedOperation.meta.id === awaitedOperation.meta.id) {
        ...
        const [onceTransformedReceivedOperation, transformedAwaitedOperation] =
          transformOperation(
            transformationFunction,
            receivedOperation,
            awaitedOperation,
          )
        const [twiceTransformedReceivedOperation, transformedBuffer] =
          transformOperation(
            transformationFunction,
...
function clientReceiveOperation<SnapshotT, OpT>(
  ...
  const newClientState: ClientAndSocketsVisualizationState<SnapshotT, OpT> = {
    ...
    toServer:
      operationToSendToServer === undefined
        ? toServer
        : [...toServer, operationToSendToServer],
...
export function onClientReceive<SnapshotT, OpT>(
  ...
  const { newClientState, transformedReceivedOperationToApply } =
    clientReceiveOperation(functions, clientLens.get(visualizationState))
